...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $('input[name^="wp_emoji_reactions_enabled_emojis"]').on('change', function() {
            updatePreview();
        });

        function updatePreview() {
            const previewContainer = $('.wp-emoji-reactions-buttons');
            previewContainer.empty();

            // Get all checked emojis
            $('input[name^="wp_emoji_reactions_enabled_emojis"]:checked').each(function() {
            ...
            if (previewContainer.children().length === 0) {
                previewContainer.append('<p>' + wp_emoji_reactions_admin.no_emojis_selected + '</p>');
            }
        }
